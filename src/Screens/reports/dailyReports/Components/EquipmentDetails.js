...
export default function EquipmentDetails({ dailyEntry, setDailyEntry }) {
    const navigation = useNavigation();
    const [modalVisible, setModalVisible] = useState(false);
    const [currentPicker, setCurrentPicker] = useState("");
    const [equipments, setEquipments] = useState(() => {
        // Initialize with a default object if no equipments exist.
        if (dailyEntry.equipments && dailyEntry.equipments.length > 0) {
            return dailyEntry.equipments;
        } else {
    ...
    }, [equipments, setDailyEntry]); // Run whenever `equipments` changes
    
    // useEffect(()=>{
    //     console.log("date : ",moment(1744304805567).format('YYYY-MM-DD hh:mm:ss ')) 
    // },[])
